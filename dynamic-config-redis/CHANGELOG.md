...
- **This store answers the engine's watch contract.** `watch_capability()`
  reports `Native` and `RemoteSource::watch` runs keyspace notifications:
  the `__keyspace@` channel, which the server publishes only when it is
  configured to — checked at startup rather than waited on.
