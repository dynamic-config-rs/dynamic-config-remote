...
- **This store answers the engine's watch contract.** `watch_capability()`
  reports `Native` and `RemoteSource::watch` runs a blocking query on
  `X-Consul-Index`: the index the agent returns moves when the key does,
  and a query holding it open returns the moment it moves.
